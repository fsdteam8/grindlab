          ...
          {items.map((item, i) => (
            <div
              ...
              className={cn(
                ...
                pv === 1 ? "basis-full" : "",
                pv === 2 ? "basis-1/2" : "",
                pv >= 3 ? "basis-1/4" : "" // we clamp to 1/4 for desktop as per screenshot
              )}
              style={{
                // ensure crisp width calculation across breakpoints
                minWidth: pv === 1 ? "100%" : pv === 2 ? "50%" : "25%",
              }}
            >
              {renderItem(item, i)}
            </div>
      ...
      {controls === "bottom" ? (
        <div className="mt-6 flex w-full items-center justify-center gap-4">
          <button
            aria-label="Previous"
            onClick={prev}
            className="inline-flex size-8 items-center justify-center rounded-full border bg-red-500 text-[var(--brand)] hover:bg-[var(--brand)] hover:text-black transition-colors"
          >
